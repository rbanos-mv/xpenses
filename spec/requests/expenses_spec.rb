...
RSpec.describe 'Expenses', type: :request do
  ...
  describe 'action expenses#create' do
    context 'with valid parameters' do
      ...
      before(:each) { post group_expenses_path(group.id), params: }

      it 'redirect to /expenses' do
        expect(response).to redirect_to(group_expenses_path(group.id))
      end
      ...
      it 'has correct placeholder text' do
        expect(response.body).to include(group_expenses_path(group.id))
      end
    end
  end

  describe 'action expenses#index' do
    before(:each) { get group_expenses_path(group.id) }

    context 'when NOT logged in' do
      before(:each) do
        logout
        get group_expenses_path(group.id)
      end
  ...
  describe 'action expenses#new' do
    before(:each) { get new_group_expense_path(group.id) }
  ...
  describe 'action expenses#show' do
    ...
    before(:each) { get group_expense_path(group.id, expense.id) }
